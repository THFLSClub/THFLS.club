---
title: OpenWrt IPv6 设置方案
date: 2025-10-18 11:06:22
tags:
---
...
具体原因见：[关于旁路由的一些吐槽](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/关于“旁路由”的一些吐槽)
...
![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/v6-7.png)
...
![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/v6-1.png)

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/v6-2.png)

- 在 `WAN 接口`的 `DHCP` 中检查设置，确保 `DHCP > IPv6 设置`已经关闭

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/v6-3.png)

- 保存并应用设置后，你的接口界面中应该会出现一个虚拟的 wan_6接口。注意此接口是无法编辑设置的。
- 确认红框中的 `IPv6-PD` 地址，获取到了这个地址才能进行下一步操作。

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/wan.png)
...
![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/wan6-1.png)

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/wan6-2.png)

保存并应用配置后，检查 WAN6 接口是否取得了 `IPv6-PD` 地址。

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/wan2.png)
...
![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/v6-4.png)
...
这里参考了如下设置：[immortalwrt/user-FAQ/如何优雅的使用IPV6？](https://github.com/immortalwrt/user-FAQ/blob/main/immortalwrt 常见问题指北.md#5-如何优雅的使用ipv60)

在 `LAN > 高级设置 > IPv6 后缀` 中填入 `eui64`

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/eui64.png)

EUI-64 可以让设备的后缀地址由 MAC 地址生成，因此可以生成唯一的后缀。

EUI-64 网络地址分配方式的技术解释可以看 ImmortalWrt 仓库的文档：[immortalwrt/user-FAQ/关于eui64的一些说明](https://github.com/immortalwrt/user-FAQ/blob/main/关于eui64的一些说明.md)

同时我们需要禁止 OpenWrt 通告 IPv6 地址的 DNS，因为设备只需要 OpenWrt 的 IPv4 DNS 地址即可实现 IPv6 解析。

![192.168.1.1 为 OpenWrt的 IPv4 地址](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/nslookup.png)

强迫下游设备使用 OpenWrt 的 IPv4 地址（比如192.168.1.1）来解析包括 IPv6 域名在内的全部域名，防止 IPv6 DNS 抢答解析请求，不会影响设备正常访问 IPv6 域名，同时可以避免发生一些相关的问题。

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/v6-5.png)

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/v6-6.png)
...
![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/ipv6test.png)
...
![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/chrome.png)

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/openwrt/pics/firefox.png)
...
![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/lede/pics/lede6-1.png)

![img](https://github.com/Aethersailor/Custom_OpenClash_Rules/wiki/doc/ipv6/lede/pics/dhcpv6.png)
...
具体设置参考：[immortalwrt/user-FAQ/IPV6如何正确配置端口转发？](https://github.com/immortalwrt/user-FAQ/blob/main/immortalwrt 常见问题指北.md#6-ipv6如何正确配置端口转发)
